...
# Created all main dictionaries and lists
user_details = {} 
...
def ordering(): # Defines the function ordering()
    ...
    while True: # Creates a while loop which will continue to loop until it is stopped by break
        try: # Trys the following code
            ...
            order_amount = order_count # Creates the variable order_amoutn and make it equal to order_count
    ...
    for item in range(order_count): # Runs the same amount of times as the order_count variable
        while order_count > 0: # Creates a while loop which runs as long as order_count is greater then zero
            ...
            HIGH = (len(menu_items)+1) # Creates th constant HIGH and makes it the amount og itmes in the menu
            question = ("please enter a number from the menu to order your food ") # Makes the question variable equal to the new question
...
def deliverycharge(): # Defines the function deliverycharge
    if order_amount < 5 and delivery_type == "delivery": # Checks if order amount is less then
...
def confirm_cancel():
    ...
    LOW = 1
    HIGH = 2
    question = ("Please type 1 or 2 ")
    confirmation = while_true(LOW,HIGH,question)
    if confirmation == 1:
...
def exit():
    ...
    restart = while_true(LOW,HIGH,question)
    if restart == 1:
        print("Exit")
        sys.exit()
    elif restart == 2:
